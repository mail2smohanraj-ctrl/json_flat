...
# Set to False (or pass --no-infer-numeric-strings on the command line) to
# disable content-based sniffing and only ever trust the JSON's own type
# (i.e. revert to the old, stricter-but-often-all-VARCHAR behavior).
INFER_TYPES_FROM_STRING_CONTENT = True
...
table_column_str_stats = defaultdict(lambda: defaultdict(lambda: {
    "total": 0, "int_like": 0, "float_like": 0, "bool_like": 0,
    "date_like": 0, "timestamp_like": 0, "leading_zero": False,
}))  # table -> col -> content-sniffing counters (see infer_snowflake_type)
...
def has_leading_zero_issue(s):
    """True for pure-integer-looking strings with a leading zero, e.g. "00501"
    or "0042" - these are almost always codes/IDs, not numbers, and would
    lose meaningful digits if cast to NUMBER. "0" itself is fine."""
    return len(s) > 1 and s[0] == "0" and INT_RE.match(s) is not None
    ...
    for col, val in row.items():
        ...
        if val is not None:
            ...
            if isinstance(val, str):
                ...
                if INFER_TYPES_FROM_STRING_CONTENT:
                    stats = table_column_str_stats[table_name][col]
                    stats["total"] += 1
                    if INT_RE.match(val):
                        stats["int_like"] += 1
                        if has_leading_zero_issue(val):
                            stats["leading_zero"] = True
                    elif FLOAT_RE.match(val):
                        stats["float_like"] += 1
                    if val.lower() in BOOL_STRINGS:
                        stats["bool_like"] += 1
                    if TIMESTAMP_RE.match(val):
                        stats["timestamp_like"] += 1
...
def infer_snowflake_type(table, col):
    ...
    # Pure-string column: the JSON author may have stringified numbers/
    # booleans/dates (very common in real-world exports). Sniff the actual
    # content before giving up and calling it VARCHAR.
    if types_seen == {str} and INFER_TYPES_FROM_STRING_CONTENT:
        stats = table_column_str_stats[table][col]
        total = stats["total"]
        if total > 0:
            if stats["bool_like"] == total:
                return "BOOLEAN"
            if stats["int_like"] == total and not stats["leading_zero"]:
                return "NUMBER(38,0)"
            if stats["int_like"] + stats["float_like"] == total and not stats["leading_zero"]:
                return "FLOAT"
            if stats["timestamp_like"] == total:
                return "TIMESTAMP_NTZ"
            if stats["date_like"] == total:
                return "DATE"

    # Anything involving genuinely mixed/non-numeric text -> VARCHAR (safe fallback)
...
def main():
    global INFER_TYPES_FROM_STRING_CONTENT
    args = sys.argv[1:]
    if "--no-infer-numeric-strings" in args:
        INFER_TYPES_FROM_STRING_CONTENT = False
        args = [a for a in args if a != "--no-infer-numeric-strings"]

    if len(args) < 2:
        print(f"Usage: {sys.argv[0]} [--no-infer-numeric-strings] <output_dir> <input_json_file_or_dir> [more files/dirs ...]")
        sys.exit(1)
